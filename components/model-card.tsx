  ...
  return (
    <Card className="w-full hover:shadow-md transition-all flex flex-col aspect-square">
      <CardHeader className="pb-2 flex-shrink-0">
        <CardTitle
          className="text-lg md:text-base font-medium truncate"
          title={model.name}
        >
          {model.name}
        </CardTitle>
        ...
        <div className="flex flex-wrap items-center gap-1 mt-1">
          {model.author && (
            <Button
              variant="outline"
              className="text-xs md:text-[10px] px-1.5 py-0.5 h-auto"
              size="sm"
              title={model.author}
            >
              <span className="truncate max-w-[100px]">{model.author}</span>
            </Button>
          )}

          <Button
            variant="outline"
            className="text-xs md:text-[10px] px-1.5 py-0.5 h-auto font-mono text-bold"
            size="sm"
          >
          ...
          {model.hf_id && (
            <Button
              variant="outline"
              className="text-xs md:text-[10px] p-1 rounded-full h-auto"
              size="sm"
            >
              <Link
                ...
                target="_blank"
              >
                <HFIcon className="size-3.5 md:size-3.5" />
              </Link>
            </Button>
          )}
        </div>
        ...
        <div className="w-full h-full bg-gradient-to-br from-[#f8f9fa] to-[#f1f3f5] dark:from-[#2a2a2a] dark:to-[#222222] rounded-lg relative">
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <div className="w-full flex flex-col items-center gap-3 z-10">
              <div className="flex flex-col items-center transform -translate-x-[35%] md:-translate-x-[20%] lg:-translate-x-[25%] 2xl:-translate-x-[40%] -translate-y-2 md:-translate-y-0.5 lg:-translate-y-2 2xl:-translate-y-3">
                <span className="text-xs md:text-[10px] lg:text-xs text-muted-foreground font-medium">
                  Input
                </span>
                <div className="font-mono text-4xl md:text-lg xl:text-4xl 2xl:text-3xl font-bold">
